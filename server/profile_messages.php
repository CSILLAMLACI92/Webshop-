...
if ($method === "GET") {
    $viewer = verify_jwt();
    $viewerId = $viewer && isset($viewer->id) ? (int)$viewer->id : 0;
    ...
    while ($row = $res->fetch_assoc()) {
        $list[] = [
            ...
            "can_delete" => ($viewerId > 0 && ($viewerId === $user_id || $viewerId === (int)($row["author_user_id"] ?? 0)))
        ];
    }
...
if ($method === "DELETE") {
    ...
    $uid = (int)$user->id;
    $stmt = $conn->prepare("DELETE FROM profile_messages WHERE id = ? AND (author_user_id = ? OR target_user_id = ?) LIMIT 1");
    $stmt->bind_param("iii", $messageId, $uid, $uid);
    $stmt->execute();
